...
_0x0:
	...
	.DB  0x79,0x20,0x63,0x61,0x6C,0x63,0x75,0x6C
	.DB  0x61,0x74,0x65,0x64,0x20,0x3D,0x20,0x25
	.DB  0x64,0x0,0x63,0x61,0x6C,0x63,0x75,0x6C
	.DB  0x61,0x74,0x69,0x6E,0x67,0x0
_0x2000003:
	.DB  0x80,0xC0
...
; .FSTART _timer1_ovf_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 0028     ++timerCounterForOverflow;
	MOVW R30,R4
	ADIW R30,1
	MOVW R4,R30
; 0000 0029     if(timerCounterForOverflow == 100){
	LDI  R30,LOW(100)
	LDI  R31,HIGH(100)
	CP   R30,R4
	CPC  R31,R5
...
; 0000 002B         TCNT1=0;
	...
	OUT  0x2C+1,R31
	OUT  0x2C,R30
; 0000 002C         TIFR = (1<<ICF1);
	LDI  R30,LOW(32)
	OUT  0x38,R30
; 0000 002D         //calculating begin
; 0000 002E         TCCR1B = 0x41;
	LDI  R30,LOW(65)
	OUT  0x2E,R30
; 0000 002F         while ((TIFR&(1<<ICF1)) == 0);
_0x4:
	IN   R30,0x38
	SBRS R30,5
	RJMP _0x4
; 0000 0030         begin = ICR1;
	CALL SUBOPT_0x0
	MOVW R6,R30
; 0000 0031         TIFR = (1<<ICF1);
	LDI  R30,LOW(32)
	OUT  0x38,R30
; 0000 0032         //calculating end
; 0000 0033         TCCR1B = 0x41;
	LDI  R30,LOW(65)
	OUT  0x2E,R30
...
; 0000 0038         TCCR1B = 0;
	...
	OUT  0x2E,R30
; 0000 0039 
; 0000 003A 		if(begin < end)
	__CPWRR 6,7,8,9
	BRGE _0xA
; 0000 003B 		{
; 0000 003C             lcd_clear();
	RCALL _lcd_clear
; 0000 003D 			timeLenght = end - begin;
	MOVW R30,R8
	SUB  R30,R6
	SBC  R31,R7
	MOVW R10,R30
; 0000 003E 			freq= 8000000UL/timeLenght;
	CALL __CWD1
	__GETD2N 0x7A1200
	CALL __DIVD21U
	STS  _freq,R30
	...
	STS  _freq+2,R22
	STS  _freq+3,R23
; 0000 003F             sprintf(frequencyChars,"Frequency calculated = %d", (int)freq);
	LDI  R30,LOW(_frequencyChars)
	LDI  R31,HIGH(_frequencyChars)
	ST   -Y,R31
	ST   -Y,R30
	__POINTW1FN _0x0,0
	ST   -Y,R31
	...
	CALL _sprintf
	ADIW R28,8
; 0000 0040 			lcd_puts(frequencyChars);
	LDI  R26,LOW(_frequencyChars)
	LDI  R27,HIGH(_frequencyChars)
	RJMP _0x11
; 0000 0041 		}
; 0000 0042         else {
_0xA:
; 0000 0043             lcd_puts("calculating");
	__POINTW2MN _0xC,0
_0x11:
	RCALL _lcd_puts
; 0000 0044         }
...
; 0000 005D // Timer Period: 0.52429 s
...
; 0000 0063 TCCR1B=0x01;
	LDI  R30,LOW(1)
	OUT  0x2E,R30
; 0000 0064 TCNT1H=0xFF;
	LDI  R30,LOW(255)
	OUT  0x2D,R30
; 0000 0065 TCNT1L=0xF0;
	LDI  R30,LOW(240)
	OUT  0x2C,R30
; 0000 0066 
; 0000 0067 
...
; 0000 0071 TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (1<<TOIE1) | (0<<OCIE0) | (0<<TOIE0);
	LDI  R30,LOW(4)
	OUT  0x39,R30
; 0000 0072 
; 0000 0073 #asm("sei")
